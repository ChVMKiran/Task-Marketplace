...
function ToggleSwitch({ enabled = false }: { enabled?: boolean }) {
  return (
    <div className={`w-10 h-6 rounded-full flex items-center px-0.5 cursor-pointer transition-colors ${enabled ? "bg-neutral-100" : "bg-white"}`}>
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="glass-card rounded-2xl p-6 space-y-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div><label className="text-sm text-neutral-600 mb-1.5 block">Full Name</label><input type="text" defaultValue="Arjun Mehta" className="input-field" /></div>
          <div><label className="text-sm text-neutral-600 mb-1.5 block">Email</label><input type="email" defaultValue="arjun@campuscraft.io" className="input-field" /></div>
          <div><label className="text-sm text-neutral-600 mb-1.5 block">Department</label><input type="text" defaultValue="Computer Science" className="input-field" /></div>
          <div><label className="text-sm text-neutral-600 mb-1.5 block">Year</label><input type="text" defaultValue="3rd Year" className="input-field" /></div>
        </div>
        <div><label className="text-sm text-neutral-600 mb-1.5 block">Bio</label><textarea rows={3} defaultValue="Creative developer and designer." className="input-field resize-none" /></div>
