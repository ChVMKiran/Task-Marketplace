  ...
  return (
    <div className="max-w-6xl mx-auto">
      <div className="mb-6">
        ...
        <p className="text-zinc-400 text-sm mt-1">Communicate with clients and contributors</p>
      </div>
            ...
            {mockConversations.map((conv) => (
              ...
              >
                <div className="w-10 h-10 rounded-full bg-gradient-to-br from-violet-500 to-indigo-600 flex items-center justify-center text-white text-xs font-semibold shrink-0">
                  {conv.participants[1].name.split(" ").map(n => n[0]).join("")}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between">
                    <span className="text-sm font-medium text-white truncate">{conv.participants[1].name}</span>
                    <span className="text-[10px] text-zinc-500 shrink-0">{formatRelativeTime(conv.updatedAt)}</span>
                  </div>
                  <div className="text-xs text-zinc-500 truncate mt-0.5">{conv.taskTitle}</div>
                  <div className="text-xs text-zinc-400 truncate mt-0.5">{conv.lastMessage.content}</div>
                </div>
                {conv.unreadCount > 0 && (
        ...
        <div className={`flex-1 flex flex-col ${!mobileShowChat ? "hidden md:flex" : "flex"}`}>
          ...
          <div className="h-16 px-5 flex items-center justify-between border-b border-white/5 shrink-0">
            <div className="flex items-center gap-3">
              <button onClick={() => setMobileShowChat(false)} className="md:hidden text-zinc-400 mr-1"><ArrowLeft className="w-5 h-5" /></button>
              <div className="w-9 h-9 rounded-full bg-gradient-to-br from-violet-500 to-indigo-600 flex items-center justify-center text-white text-xs font-semibold">
                {selectedConv.participants[1].name.split(" ").map(n => n[0]).join("")}
              </div>
              <div>
                <div className="text-sm font-medium text-white">{selectedConv.participants[1].name}</div>
                <div className="text-xs text-zinc-500">{selectedConv.taskTitle}</div>
              </div>
            </div>
            <div className="flex items-center gap-2">
              <button className="w-8 h-8 rounded-lg hover:bg-white/5 flex items-center justify-center text-zinc-400"><Phone className="w-4 h-4" /></button>
              <button className="w-8 h-8 rounded-lg hover:bg-white/5 flex items-center justify-center text-zinc-400"><Video className="w-4 h-4" /></button>
              <button className="w-8 h-8 rounded-lg hover:bg-white/5 flex items-center justify-center text-zinc-400"><MoreVertical className="w-4 h-4" /></button>
            </div>
          </div>
            ...
            {mockMessages.map((msg) => {
              ...
              return (
                <motion.div key={msg.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className={`flex ${isMe ? "justify-end" : "justify-start"}`}>
                  <div className={`max-w-[75%] rounded-2xl px-4 py-2.5 ${isMe ? "bg-violet-500/20 border border-violet-500/20" : "bg-zinc-800/60 border border-white/5"}`}>
                    <p className="text-sm text-zinc-200">{msg.content}</p>
                    <p className="text-[10px] text-zinc-500 mt-1 text-right">{formatRelativeTime(msg.createdAt)}</p>
                  </div>
                </motion.div>
              );
            })}
          ...
          <div className="p-4 border-t border-white/5 shrink-0">
            <div className="flex items-center gap-2">
              <button className="w-9 h-9 rounded-xl bg-zinc-800/50 hover:bg-zinc-700/50 flex items-center justify-center text-zinc-400"><Paperclip className="w-4 h-4" /></button>
              <input
                type="text"
                value={messageInput}
