    ...
    <div className="max-w-5xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-neutral-600 text-sm mt-1">Manage all your posted tasks</p>
        </div>
      ...
      <div className="glass-card rounded-2xl overflow-hidden">
        ...
        <div className="divide-y divide-neutral-200">
          {mockTasks.map((task, i) => (
            <Link key={task.id} href={`/tasks/${task.id}`} className="flex items-center gap-4 px-5 py-4 hover:bg-neutral-100/50 transition-colors">
              <div className={`w-2.5 h-2.5 rounded-full shrink-0 ${
                task.status === "open" ? "bg-neutral-100" : task.status === "in-progress" ? "bg-yellow-400" : "bg-emerald-400"
              }`} />
              ...
              <span className="text-sm font-semibold text-black">{formatCurrency(task.budget)}</span>
