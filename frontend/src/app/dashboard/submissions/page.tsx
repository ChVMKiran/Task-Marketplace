    ...
    <div className="max-w-5xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-neutral-600 text-sm mt-1">Track all your submissions and their status</p>
        </div>
        ...
        <div className="divide-y divide-neutral-200">
          {allSubs.map((sub, i) => (
            <motion.div
              ...
              className="flex items-center gap-4 px-5 py-4 hover:bg-neutral-100/50 transition-colors"
            >
              <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${
                sub.status === "winner" ? "bg-neutral-100" :
                sub.status === "shortlisted" ? "bg-yellow-500/10" :
                sub.status === "accepted" ? "bg-emerald-500/10" :
                sub.status === "rejected" ? "bg-red-500/10" : "bg-neutral-100/50"
              }`}>
                {sub.status === "winner" ? <Trophy className="w-5 h-5 text-neutral-900" /> :
